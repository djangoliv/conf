...
#+BEGIN_SRC emacs-lisp
  ...
  (defun delete-enclosed-text ()
    ...
    (save-excursion
      (let (p1 p2 porig)
        ;; where I am (init loop)
        (setq porig (point))
        (setq p1 (point))
        ;; end-of-buffer
        (save-excursion  (end-of-buffer) (setq eob (point)))
        (setq p2 eob)
        ;; test if there is nothing between delimiters => delete delimiters
        (setq mychar (thing-at-point 'char))
        (backward-char -1)
        (if (string-equal (thing-at-point 'char) (get-enc-char mychar))
            (progn
              (setq case "if")
              (backward-char -1)
              (setq p2 (point))
          ...
          (progn
            (setq case "else")
            (backward-char 1)
            (while (and (equal p2 eob)) (not (equal p1 0))
                   (message "cas2")
                   (goto-char p1)
                   (backward-char 1)
                   (skip-chars-backward "^({[<>\"'" )
                   (setq p1 (point))
                   (backward-char 1) (setq mychar (thing-at-point 'char)) (goto-char porig)
                   (skip-chars-forward (concatenate 'string "^" (get-enc-char mychar)))
                   (setq p2 (point)))
            (delete-region p1 p2)))))
    (if (equal case "else")
        (backward-char 1)))
