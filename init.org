* Emacs Configuration                                      :noexport:ARCHIVt:
#+AUTHOR: Djangoliv'
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Read subprocess output in 1 MiB chunks (default 64 KiB): terminals such as
  ;; eat emit large bursts, fewer reads means fewer redisplays
  ...
  ;; Un gros fichier est superieur à 30mb
  ...
  ;; alias
  (defalias `line-number `display-line-numbers-mode)
  (defalias `number-line `display-line-numbers-mode)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package tablist :straight t)
...
#+BEGIN_SRC emacs-lisp
  (use-package beacon
    ...
    ;; eat moves point on every chunk of output: the blink overlay would be
    ;; re-created endlessly and can get stuck as a blue highlight
...
#+BEGIN_SRC emacs-lisp
  (use-package dimmer
    ...
    ;; Terminal emulator buffers must never be dimmed.  `dimmer-dim-buffer'
    ;; pushes one `face-remap-add-relative' per face of `face-list' (~1500
    ;; here) into the buffer's `face-remapping-alist'.  A term/eat buffer
    ;; carries a face property on nearly every character and repaints its
    ;; whole screen many times per second, so redisplay crawls -- and the
    ;; already dim ANSI colors of a TUI become unreadable.
    (defun my/dimmer-terminal-buffer-p (buffer)
      "Return non-nil if BUFFER runs a terminal emulator."
      (with-current-buffer buffer
        (derived-mode-p 'term-mode 'eat-mode 'vterm-mode)))
    (add-to-list 'dimmer-buffer-exclusion-predicates
                 #'my/dimmer-terminal-buffer-p)
    ;; Emacs 31.1 introduced `whitespace-page-delimiter', whose :underline
    ;; spec is (:color foreground-color :style double-line).  The symbol
    ;; `foreground-color' is a valid face color, but dimmer feeds it straight
    ;; to `color-defined-p', which signals (wrong-type-argument stringp
    ;; foreground-color) on every redisplay.  Skip attributes whose color is
    ;; not a literal color string; they follow the foreground, already dimmed.
    (defun my/dimmer-skip-symbolic-color (fn face attribute &rest args)
      "Around advice for `dimmer--dim-face-attribute'.
Return nil when FACE's ATTRIBUTE carries a symbolic rather than
literal color, instead of letting dimmer signal an error."
      (let ((value (face-attribute face attribute nil t)))
        (unless (and (consp value)
                     (plist-member value :color)
                     (not (stringp (plist-get value :color))))
          (apply fn face attribute args))))
    (advice-add 'dimmer--dim-face-attribute :around
                #'my/dimmer-skip-symbolic-color)
    (dimmer-mode)
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package exec-path-from-shell
    :straight t
    :init
    (setq exec-path-from-shell-variables '("PATH" "PYTHONPATH" "WORKON_HOME"))
...
** Registres
  #+BEGIN_SRC emacs-lisp
    ...
    (set-register ?s '(file . "~/.emacs-org/song.org"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (defun djangoliv--org-open-at-point-generic-browser (orig-fun &rest args)
      "Open links with `browse-url-generic'."
      (let ((browse-url-browser-function 'browse-url-generic))
        (apply orig-fun args)))
    (advice-add 'org-open-at-point :around
                #'djangoliv--org-open-at-point-generic-browser)
    ;; keywords
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; plantuml
  (setq org-plantuml-jar-path "~/Tools/plantuml/plantuml.jar")
...
#+BEGIN_SRC emacs-lisp
  (use-package jupyter
    ...
    :after org
    :config
    ;; jupyter must come last in org-babel-load-languages
    (org-babel-do-load-languages
     'org-babel-load-languages
     (append org-babel-load-languages '((jupyter . t))))
    ;; blocks are #+BEGIN_SRC jupyter-python, not ipython
...
#+BEGIN_SRC emacs-lisp
  (use-package org-re-reveal
    ...
    :defer 2
    :config
    (setq org-re-reveal-root (concat "file://" (expand-file-name "~/Tools/revealjs/reveal.js/")))
...
#+BEGIN_SRC emacs-lisp
  (use-package yasnippet
    ...
    (defun djangoliv--yas-completing-prompt (orig-fun &rest args)
      "Use `yas/completing-prompt' for `yas/prompt-functions' but only here..."
      (let ((yas-prompt-functions '(yas/completing-prompt)))
        (apply orig-fun args)))
    (advice-add 'yas-insert-snippet :around #'djangoliv--yas-completing-prompt)
...
*** elisp
#+BEGIN_SRC emacs-lisp
  (add-to-list 'auto-mode-alist '("\\.emacs-terms\\'" . emacs-lisp-mode))
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'auto-mode-alist '("\\pylintrc\\'" . ini-mode))
  ...
  (add-hook 'python-ts-mode-hook
            (lambda ()
              ...
              (global-set-key (kbd "C-.") 'xref-find-definitions)
              (setq flycheck-pycheckers-flake8-command
...
#+BEGIN_SRC emacs-lisp
  (use-package python-environment
    :straight t
    :config
    (setq python-environment-directory "~/Tools/python/venv/"))
#+END_SRC
**** docstring
#+BEGIN_SRC emacs-lisp
...
**** pyvenv
#+BEGIN_SRC emacs-lisp
  (setenv "WORKON_HOME" (expand-file-name "~/Tools/python/venv"))
  (use-package pyvenv
    :straight t
    :config
    ;; conda base: same environment as the interactive shell
    (pyvenv-activate (expand-file-name "~/Tools/miniforge3"))
  ...
  (add-hook 'context-menu-functions
            (lambda (menu click)
              (easy-menu-add-item menu nil
                                  ["Rename" eglot-rename t])
              (easy-menu-add-item menu nil
                                  ["Find references" xref-find-references t])
              (easy-menu-add-item menu nil
                                  ["Code actions" eglot-code-actions t])
              menu))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package web-mode
    ...
    :config
    (setq web-mode-content-types-alist '(("jsx" . "\\.[c|m]js[x]?\\'")))
    ;; hook
    (add-hook 'web-mode-hook
              (lambda ()
                ;; indentation
  ...
  ;; "Active prettier-js-mode uniquement pour les fichiers JS et TSX."
  (defun my/prettier-js-for-web-mode ()
    (let ((file-ext (file-name-extension buffer-file-name)))
      (when (member file-ext '("js" "tsx" "ts" "jsx"))
        (prettier-js-mode))))
  (add-hook 'web-mode-hook #'my/prettier-js-for-web-mode)
  #+END_SRC
**** emmet
#+BEGIN_SRC emacs-lisp
  ...
  (use-package centaur-tabs
    ...
    ("C-<prior>" . centaur-tabs-backward)
      ...
      (defun centaur-tabs-hide-tab (x)
        (let ((name (format "%s" x)))
          (and
           ;; keep the claude buffers, despite their leading star
                  ...
                  (if (file-exists-p (replace-regexp-in-string "$i" "0" path ))
                    ...
                    (if (file-exists-p (replace-regexp-in-string "a/" "" path ))
                      ...
                      (if (file-exists-p (replace-regexp-in-string "b/" "" path ))
                        ...
                        (if (file-exists-p (concat "~/workspaces/jupyterlab-projects/jupyterhub-projects/jupyterApps/app" path))
                            (find-file (concat "~/workspaces/jupyterlab-projects/jupyterhub-projects/jupyterApps/app" path))
                          (if (file-exists-p (concat "~/workspaces/" path))
                              (find-file (concat "~/workspaces/" path))
...
#+BEGIN_SRC emacs-lisp
  (use-package ediff
    ...
    (defun djangoliv--ediff-save-windows (orig-fun &rest args)
      "Save the window configuration before calling ORIG-FUN."
      (setq ediff-last-windows (current-window-configuration))
      (add-hook 'ediff-after-quit-hook-internal 'ediff-restore-windows)
      (apply orig-fun args))
    (advice-add 'ediff-buffers :around #'djangoliv--ediff-save-windows))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; close categories
  (defvar my/recentf-tree-open-by-default nil
    "Non-nil means tree categories in recentf menu are open by default.")

  (defun my/recentf-open-files-item (menu-element)
    "Return a widget to display MENU-ELEMENT in a dialog buffer.
    This is a replacement for `recentf-open-files-item` allowing collapsed trees."
    (if (consp (cdr menu-element))
        ;; Represent a sub-menu with a tree widget
        `(tree-widget
          :open ,my/recentf-tree-open-by-default
          :match ignore
          :node (item :tag ,(car menu-element)
                      :sample-face bold
                      :format "%{%t%}:\n")
          ,@(mapcar #'my/recentf-open-files-item
                    (cdr menu-element)))
      ...
      `(link :tag ,(car menu-element)
             ...
             ,(cdr menu-element))))
  ;; Surcharge de la fonction originale
  (advice-add 'recentf-open-files-item :override #'my/recentf-open-files-item)
  ;; open
  (defun my/recentf-open-all-categories ()
    "Ouvre toutes les catégories dans recentf."
    (interactive)
    ;; Force toutes les catégories à être ouvertes
    (let ((my/recentf-tree-open-by-default t))
      ;; Ferme le buffer existant s'il est ouvert
      (when (get-buffer "*Recentf*")
        (kill-buffer "*Recentf*"))
      ;; Ouvre le menu recentf avec toutes les catégories ouvertes
      (recentf-open-files)))
  (with-eval-after-load 'recentf
    (define-key recentf-dialog-mode-map (kbd "M-o") #'my/recentf-open-all-categories))
#+END_SRC
*** ibuffer
 #+BEGIN_SRC emacs-lisp
   ;; ibuffer
...
#+BEGIN_SRC emacs-lisp
  (use-package languagetool
    ...
    (setq languagetool-console-command
          (expand-file-name "~/Tools/languageTool/languagetool-6.6/languagetool-commandline.jar"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq browse-url-generic-program (expand-file-name "~/Tools/firefox/firefox/firefox")
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'load-path "~/.emacs.d/neotree") ;; treemacs ?
  (require 'neotree)
  (global-set-key [f8] 'neotree-toggle)
  (setq flycheck-python-flake8-args "--ignore=D100,D101,D102,D103,D104,D105,D106")
  (use-package just-mode :straight t)
  
  (setq inhibit-locking t)
 #+END_SRC
* Claude
#+BEGIN_SRC emacs-lisp
  ...
  (use-package claude-code
        ...
        (add-hook 'claude-code-start-hook (lambda () (setq-local show-trailing-whitespace nil))) 
        (monet-mode 1)
        (claude-code-mode)
        :bind-keymap ("C-c c" . claude-code-command-map))

  (defun my-claude-display-bottom (buffer)
      (display-buffer
         buffer
         '((display-buffer-in-side-window)
           (side . bottom)
           (window-height . 0.40))))
  (setq claude-code-display-window-fn #'my-claude-display-bottom)

  ;; notif TEST
  (defun my/claude-mark-busy ()
      (with-temp-file "/tmp/claude_busy")
      (when (file-exists-p "/tmp/claude_done")
        (delete-file "/tmp/claude_done")))
  (add-hook 'claude-code-request-start-hook #'my/claude-mark-busy)
  (defun my/claude-mark-done ()
      (with-temp-file "/tmp/claude_done")
    (when (file-exists-p "/tmp/claude_busy")
      (delete-file "/tmp/claude_busy")))
  (add-hook 'claude-code-response-finished-hook #'my/claude-mark-done)
  ;; desable click on eat
  (with-eval-after-load 'eat
    (dolist (map (list eat-semi-char-mode-map eat-char-mode-map))
      (define-key map [mouse-1] #'mouse-select-window)
      (define-key map [down-mouse-1] #'mouse-select-window)
      ;; eat sends almost every key to the terminal: keep group navigation
      (define-key map (kbd "M-+") #'centaur-tabs-backward-group)
      (define-key map (kbd "M--") #'centaur-tabs-forward-group)))
  ;; Lighter redisplay in eat buffers: no bidi analysis, no completion popup
  (defun my-eat-mode-setup ()
    "Buffer-local settings for `eat-mode' buffers."
  ...
  ;;         (lambda ()
  ;;           (setq-local eat-enable-mouse nil)
  ;;           (when (fboundp 'eat--set-cursor) (eat-reset))))
  #+END_SRC
* ollama
#+BEGIN_SRC emacs-lisp
  (use-package llm
  ...
  (use-package ellama
    ...
    :after llm
    :init
    (setq ellama-directory
          (expand-file-name "ellama" user-emacs-directory))
    :config
    (setq ellama-provider
          (make-llm-ollama
           :chat-model "qwen2.5:7b-instruct"))
    (setq ellama-system-prompt
          "You are a precise assistant. Follow instructions exactly.
    Be concise. Never add explanations unless asked.")
    )
  (defun etranslate ()
    (interactive)
    (ellama-change
     "Detect language. Translate English to French or French to English. Output ONLY translation. No explanations.")
    )
#+END_SRC
